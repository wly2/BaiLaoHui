...
public class MyFont
{
    ...
    static void Font()
    {
        Material mtr = Resources.Load<Material>("Fonts/Num6"); //把我们创建的材质球加载进来
        Texture2D texture = Resources.Load<Texture2D>("Fonts/Num6_0"); //把我们在位图制作工具生成的图片加载进来
        mtr.SetTexture(0, texture); //把图片赋给材质球
        Font font = Resources.Load<Font>("Fonts/Num6"); //把我们创建的字体加载进来
        XmlDocument xml = new XmlDocument();
        xml.Load(Application.dataPath + "/Resources/Fonts/Num6.fnt"); //这是在BMFont里得到的那个.fnt文件,因为是xml文件，所以我们就用xml来解析
        List<CharacterInfo> chtInfoList = new List<CharacterInfo>();
        XmlNode node = xml.SelectSingleNode("font/chars");
        for (int i = 0; i < node.ChildNodes.Count; ++i)
        {
            XmlElement xe = (XmlElement) node.ChildNodes[i];
            ...
            chtInfo.uvBottomRight = new Vector2((float) (x + width) / texture.width,
