...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderXLuaTestPedding
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<WindowUIType>(translator.PushWindowUIType, translator.Get, translator.UpdateWindowUIType);
        ...
        int WindowUIType_TypeID = -1;
		int WindowUIType_EnumRef = -1;
        
        public void PushWindowUIType(RealStatePtr L, WindowUIType val)
        {
            if (WindowUIType_TypeID == -1)
            {
			    bool is_first;
                WindowUIType_TypeID = getTypeId(L, typeof(WindowUIType), out is_first);
				
				if (WindowUIType_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(WindowUIType));
				    WindowUIType_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, WindowUIType_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, WindowUIType_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for WindowUIType ,value="+val);
            }
			
			LuaAPI.lua_getref(L, WindowUIType_EnumRef);
        ...
        public void Get(RealStatePtr L, int index, out WindowUIType val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != WindowUIType_TypeID)
				{
				    throw new Exception("invalid userdata for WindowUIType");
				}
                ...
                {
                    throw new Exception("unpack fail for WindowUIType");
                }
				val = (WindowUIType)e;
            ...
            {
                val = (WindowUIType)objectCasters.GetCaster(typeof(WindowUIType))(L, index, null);
            }
        }
		
        public void UpdateWindowUIType(RealStatePtr L, int index, WindowUIType val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for WindowUIType ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(WindowUIType[]))
			{
			    WindowUIType[] array = obj as WindowUIType[];
				translator.PushWindowUIType(L, array[index]);
				return true;
			}
