...
public class RFX4_TrailRenderer : MonoBehaviour
{
    ...
    [Range(0.001f, 1)] public float MinVertexDistance = 0.01f;
    public float Gravity = 0.01f;
    public Vector3 Force = new Vector3(0, 0, 0);
    public float InheritVelocity;
    public float Drag = 0.01f;
    [Range(0.001f, 10)] public float Frequency = 1;
    [Range(0.001f, 10)] public float OffsetSpeed = 0.5f;
    public bool RandomTurbulenceOffset;
    [Range(0.001f, 10)] public float Amplitude = 2;
    ...
    private void UpdateForce()
    {
        ...
        for (int i = 0; i < positions.Count; i++)
        {
            ...
            if (TurbulenceStrength > 0.000001f)
            {
                ...
                turbulenceVel.x += ((Mathf.PerlinNoise(pos.z, pos.y) * 2 - 1) * Amplitude) * Time.deltaTime *
                                   TurbulenceStrength / 10f;
                turbulenceVel.y += ((Mathf.PerlinNoise(pos.x, pos.z) * 2 - 1) * Amplitude) * Time.deltaTime *
                                   TurbulenceStrength / 10f;
                turbulenceVel.z += ((Mathf.PerlinNoise(pos.y, pos.x) * 2 - 1) * Amplitude) * Time.deltaTime *
                                   TurbulenceStrength / 10f;
            }
