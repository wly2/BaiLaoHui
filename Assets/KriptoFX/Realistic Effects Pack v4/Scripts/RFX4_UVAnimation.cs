...
public class RFX4_UVAnimation : MonoBehaviour
{
    ...
    private void InitDefaultVariables()
    {
        ...
        if (instanceMaterial != null)
        {
            ...
            {
                instanceMaterial.SetTextureScale(TextureNames[i].ToString(), size);
                instanceMaterial.SetTextureOffset(TextureNames[i].ToString(), offset);
            }
        }
    }
        ...
        if (currentRenderer == null)
        {
            ...
            if (projector != null)
            {
                if (!projector.material.name.EndsWith("(Instance)"))
                    projector.material =
                        new Material(projector.material) {name = projector.material.name + " (Instance)"};
