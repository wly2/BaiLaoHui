...
public class RFX4_Turbulence : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            currentOffset = Application.isPlaying ? OffsetSpeed * Time.time : OffsetSpeed * Time.realtimeSinceStartup;
        }
...
#if !UNITY_5_5_OR_NEWER
            if (TurbulenceByTime)
                timeTurbulenceStrength =
 TurbulenceStrengthByTime.Evaluate(1 - particle.lifetime / particle.startLifetime);
#else
            if (TurbulenceByTime)
                timeTurbulenceStrength =
                    TurbulenceStrengthByTime.Evaluate(1 - particle.remainingLifetime / particle.startLifetime);
#endif
            ...
            turbulenceVector.x =
                ((Mathf.PerlinNoise(pos.z - currentOffset.z, pos.y - currentOffset.y) * 2 - 1) * Amplitude.x) *
                timeOffset;
            turbulenceVector.y =
                ((Mathf.PerlinNoise(pos.x - currentOffset.x, pos.z - currentOffset.z) * 2 - 1) * Amplitude.y) *
                timeOffset;
            turbulenceVector.z =
                ((Mathf.PerlinNoise(pos.y - currentOffset.y, pos.x - currentOffset.x) * 2 - 1) * Amplitude.z) *
                timeOffset;
            ...
            if (AproximatedFlyDistance > 0)
                velocityByDistanceMultiplier =
                    VelocityByDistance.Evaluate(Mathf.Clamp01(distance / AproximatedFlyDistance));
            ...
            if (MoveMethod == MoveMethodEnum.RelativePosition)
            {
                ...
                particleArray[i].velocity = particleArray[i].velocity * 0.85f +
                                            turbulenceVector.normalized * 0.15f * velocityByDistanceMultiplier +
                                            GlobalForce * velocityByDistanceMultiplier;
            }
        }
