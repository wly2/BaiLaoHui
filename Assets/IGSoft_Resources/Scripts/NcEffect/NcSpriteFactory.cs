...
public class NcSpriteFactory : NcEffectBehaviour
{
    ...
    public Rect GetSpriteUvRect(int nStriteIndex, int nFrameIndex)
    {
        ...
        if (m_SpriteList.Count <= nStriteIndex || m_SpriteList[nStriteIndex].m_FrameInfos == null ||
            m_SpriteList[nStriteIndex].m_FrameInfos.Length <= nFrameIndex)
    ...
    void CreateEffectObject()
    {
        ...
        if (transform.parent != null)
            transform.parent.SendMessage("OnSpriteListEffectFrame", m_SpriteList[m_nCurrentIndex],
        ...
        if (m_SpriteList[m_nCurrentIndex].m_bEffectInstantiate)
        {
            ...
            if (transform.parent != null)
                transform.parent.SendMessage("OnSpriteListEffectInstance", m_CurrentEffect,
                    SendMessageOptions.DontRequireReceiver);
        }
    }
    ...
    public void OnAnimationChangingFrame(NcSpriteAnimation spriteCom, int nOldIndex, int nNewIndex, int nLoopCount)
    {
        ...
        {
            if ((nOldIndex < m_SpriteList[m_nCurrentIndex].m_nEffectFrame || nNewIndex <= nOldIndex) &&
                m_SpriteList[m_nCurrentIndex].m_nEffectFrame <= nNewIndex)
                if (nLoopCount == 0 || !m_SpriteList[m_nCurrentIndex].m_bEffectOnlyFirst)
                    CreateEffectObject();
        }

        if (m_SpriteList[m_nCurrentIndex].m_AudioClip != null)
        {
            if ((nOldIndex < m_SpriteList[m_nCurrentIndex].m_nSoundFrame || nNewIndex <= nOldIndex) &&
                m_SpriteList[m_nCurrentIndex].m_nSoundFrame <= nNewIndex)
                if (nLoopCount == 0 || !m_SpriteList[m_nCurrentIndex].m_bSoundOnlyFirst)
    ...
    public bool OnAnimationLastFrame(NcSpriteAnimation spriteCom, int nLoopCount)
    {
        ...
        {
            NcSpriteAnimation ncTarSprite =
                SetSprite(m_SpriteList[m_nCurrentIndex].m_nNextSpriteIndex) as NcSpriteAnimation;
            if (ncTarSprite != null)
            {
