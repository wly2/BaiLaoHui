...
public class NcCurveAnimation : NcEffectAniBehaviour
{
    class NcComparerCurve : IComparer<NcInfoCurve>
    {
        ...
        public int Compare(NcInfoCurve a, NcInfoCurve b)
        {
            ...
            {
                val = b.m_AniCurve.Evaluate(1 - m_fEqualRange / m_fHDiv) -
                      a.m_AniCurve.Evaluate(1 - m_fEqualRange / m_fHDiv);
                if (Mathf.Abs(val) < m_fEqualRange)
                    return 0;
            }
    ...
    public class NcInfoCurve
    {
        ...
        public static string[] m_TypeName =
            {"None", "Position", "Rotation", "Scale", "MaterialColor", "TextureUV", "MeshColor"};
        ...
        public bool m_bRecursively; // olny color
        ...
        public int GetValueCount()
        {
            switch (m_ApplyType)
            {
                case APPLY_TYPE.POSITION: return 4;
                case APPLY_TYPE.ROTATION: return 4;
                case APPLY_TYPE.SCALE: return 3;
                case APPLY_TYPE.MATERIAL_COLOR: return 4;
                case APPLY_TYPE.TEXTUREUV: return 2;
                case APPLY_TYPE.MESH_COLOR: return 4;
        ...
        public string GetValueName(int nIndex)
        {
            ...
            switch (m_ApplyType)
            {
                ...
                case APPLY_TYPE.ROTATION:
                    valueNames = new string[4] {"X", "Y", "Z", "World"};
                    break;
                case APPLY_TYPE.SCALE:
                    valueNames = new string[4] {"X", "Y", "Z", ""};
                    break;
                case APPLY_TYPE.MATERIAL_COLOR:
                    valueNames = new string[4] {"R", "G", "B", "A"};
                    break;
                case APPLY_TYPE.TEXTUREUV:
                    valueNames = new string[4] {"X", "Y", "", ""};
                    break;
        ...
        public void SetDefaultValueScale()
        {
            switch (m_ApplyType)
            {
                ...
                case APPLY_TYPE.SCALE:
                    m_fValueScale = 1;
                    break;
                case APPLY_TYPE.MATERIAL_COLOR: break;
                case APPLY_TYPE.TEXTUREUV:
                    m_fValueScale = 10;
                    break;
                case APPLY_TYPE.MESH_COLOR: break;
                case APPLY_TYPE.NONE: break;
            }
        }
    ...
    [SerializeField] public List<NcInfoCurve> m_CurveInfoList;
    ...
    public override string CheckProperty()
    {
        ...
        {
            if (!m_CurveInfoList[i].m_bEnabled)
                continue;
            if (m_CurveInfoList[i].m_ApplyType == NcInfoCurve.APPLY_TYPE.TEXTUREUV)
                if (GetComponent("NcUvAnimation") == null)
                    return "SCRIPT_NEED_NCUVANIMATION";
            if (m_CurveInfoList[i].m_ApplyType == NcInfoCurve.APPLY_TYPE.MATERIAL_COLOR)
                if (!m_CurveInfoList[i].m_bRecursively &&
                    (GetComponent<Renderer>() == null ||
                     Ng_GetMaterialColorName(GetComponent<Renderer>().sharedMaterial) == null))
    ...
    public override int GetAnimationState()
    {
        ...
        if ((0 < m_fDurationTime && (m_fStartTime == 0 || !IsEndAnimation())))
            return 1;
        return 0;
    }
    ...
    void LateUpdate()
    {
        ...
        if (m_fStartTime == 0)
            return;
        if (!IsStartAnimation() && m_fDelayTime != 0)
        {
    ...
    void InitAnimation()
    {
        ...
        for (int i = 0; i < m_CurveInfoList.Count; ++i)
        {
            if (!m_CurveInfoList[i].m_bEnabled)
                continue;
            switch (m_CurveInfoList[i].m_ApplyType)
            {
                case NcInfoCurve.APPLY_TYPE.NONE: continue;
                case NcInfoCurve.APPLY_TYPE.POSITION:
                {
                    m_CurveInfoList[i].m_OriginalValue = Vector4.zero;
                    m_CurveInfoList[i].m_BeforeValue = m_CurveInfoList[i].m_OriginalValue;
                    break;
                }
                ...
                {
                    m_CurveInfoList[i].m_OriginalValue = m_Transform.localScale;
                    m_CurveInfoList[i].m_BeforeValue = m_CurveInfoList[i].m_OriginalValue;
                    break;
                }
                case NcInfoCurve.APPLY_TYPE.MATERIAL_COLOR:
                {
                    if (m_CurveInfoList[i].m_bRecursively)
                    {
                        ...
                        m_CurveInfoList[i].m_ChildOriginalColorValues = new Vector4[m_ChildRenderers.Length];
                        m_CurveInfoList[i].m_ChildBeforeColorValues = new Vector4[m_ChildRenderers.Length];
                        for (int n = 0; n < m_ChildRenderers.Length; n++)
                        {
                            ...
                            {
                                if (!m_bSavedOriginalValue)
                                {
                                    m_CurveInfoList[i].m_ChildOriginalColorValues[n] =
                                        ren.material.GetColor(m_ChildColorNames[n]);
                                    //										AddRuntimeMaterial(ren.material);
                                }
                                else
                                {
                                    ren.material.SetColor(m_ChildColorNames[n],
                                        m_CurveInfoList[i].m_ChildOriginalColorValues[n]);
                                }
                            }

                            m_CurveInfoList[i].m_ChildBeforeColorValues[n] = Vector4.zero;
                        }
                    }
                    else
                    {
                        if (GetComponent<Renderer>() != null)
                        {
                            ...
                            {
                                if (!m_bSavedOriginalValue)
                                    m_CurveInfoList[i].m_OriginalValue =
                                        GetComponent<Renderer>().material.GetColor(m_ColorName);
                                else
                                    GetComponent<Renderer>().material
                                        .SetColor(m_ColorName, m_CurveInfoList[i].m_OriginalValue);
                            }

                            m_CurveInfoList[i].m_BeforeValue = Vector4.zero;
                        }
                    }
                ...
                case NcInfoCurve.APPLY_TYPE.TEXTUREUV:
                {
                    ...
                    {
                        if (!m_bSavedOriginalValue)
                            m_CurveInfoList[i].m_OriginalValue = new Vector4(m_NcUvAnimation.m_fScrollSpeedX,
                                m_NcUvAnimation.m_fScrollSpeedY, 0, 0);
                        else
                        {
                            m_NcUvAnimation.m_fScrollSpeedX = m_CurveInfoList[i].m_OriginalValue.x;
                            m_NcUvAnimation.m_fScrollSpeedY = m_CurveInfoList[i].m_OriginalValue.y;
                        }
                    }
                ...
                {
                    float fValue = m_CurveInfoList[i].m_AniCurve.Evaluate(0);
                    Color tarColor = Color.Lerp(m_CurveInfoList[i].m_FromColor, m_CurveInfoList[i].m_ToColor, fValue);
                    if (m_CurveInfoList[i].m_bRecursively)
                    {
    ...
    void UpdateAnimation(float fElapsedRate)
    {
        ...
        for (int i = 0; i < m_CurveInfoList.Count; ++i)
        {
            if (!m_CurveInfoList[i].m_bEnabled)
                continue;
            float fValue = m_CurveInfoList[i].m_AniCurve.Evaluate(m_fElapsedRate);
            if (m_CurveInfoList[i].m_ApplyType != NcInfoCurve.APPLY_TYPE.MATERIAL_COLOR &&
                m_CurveInfoList[i].m_ApplyType != NcInfoCurve.APPLY_TYPE.MESH_COLOR)
                fValue *= m_CurveInfoList[i].m_fValueScale;
            switch (m_CurveInfoList[i].m_ApplyType)
            {
                case NcInfoCurve.APPLY_TYPE.NONE: continue;
                case NcInfoCurve.APPLY_TYPE.POSITION:
                {
                    if (m_CurveInfoList[i].m_bApplyOption[3])
                        m_Transform.position += new Vector3(GetNextValue(m_CurveInfoList[i], 0, fValue),
                            GetNextValue(m_CurveInfoList[i], 1, fValue), GetNextValue(m_CurveInfoList[i], 2, fValue));
                    else
                        m_Transform.localPosition += new Vector3(GetNextValue(m_CurveInfoList[i], 0, fValue),
                            GetNextValue(m_CurveInfoList[i], 1, fValue), GetNextValue(m_CurveInfoList[i], 2, fValue));
                    break;
                }
                case NcInfoCurve.APPLY_TYPE.ROTATION:
                {
                    if (m_CurveInfoList[i].m_bApplyOption[3])
                        m_Transform.rotation *= Quaternion.Euler(GetNextValue(m_CurveInfoList[i], 0, fValue),
                            GetNextValue(m_CurveInfoList[i], 1, fValue), GetNextValue(m_CurveInfoList[i], 2, fValue));
                    else
                        m_Transform.localRotation *= Quaternion.Euler(GetNextValue(m_CurveInfoList[i], 0, fValue),
                            GetNextValue(m_CurveInfoList[i], 1, fValue), GetNextValue(m_CurveInfoList[i], 2, fValue));
                    break;
                }
                case NcInfoCurve.APPLY_TYPE.SCALE:
                {
                    m_Transform.localScale += new Vector3(GetNextScale(m_CurveInfoList[i], 0, fValue),
                        GetNextScale(m_CurveInfoList[i], 1, fValue), GetNextScale(m_CurveInfoList[i], 2, fValue));
                    break;
                }
                case NcInfoCurve.APPLY_TYPE.MATERIAL_COLOR:
                {
                    if (m_CurveInfoList[i].m_bRecursively)
                    {
                        ...
                        for (int arrayIndex = 0; arrayIndex < m_ChildColorNames.Length; arrayIndex++)
                            if (m_ChildColorNames[arrayIndex] != null && m_ChildRenderers[arrayIndex] != null)
                                SetChildMaterialColor(m_CurveInfoList[i], fValue, arrayIndex);
                    }
                    else
                    {
                        if (GetComponent<Renderer>() != null && m_ColorName != null)
                        {
                            ...
                            Color color = m_CurveInfoList[i].m_ToColor - m_CurveInfoList[i].m_OriginalValue;
                            Color currentColor = m_MainMaterial.GetColor(m_ColorName);
                            for (int n = 0; n < 4; n++)
                                currentColor[n] += GetNextValue(m_CurveInfoList[i], n, color[n] * fValue);
                ...
                case NcInfoCurve.APPLY_TYPE.TEXTUREUV:
                {
                    if (m_NcUvAnimation)
                    {
                        m_NcUvAnimation.m_fScrollSpeedX += GetNextValue(m_CurveInfoList[i], 0, fValue);
                        m_NcUvAnimation.m_fScrollSpeedY += GetNextValue(m_CurveInfoList[i], 1, fValue);
                    }
    ...
    public NcInfoCurve GetCurveInfo(string curveName)
    {
        ...
        for (int i = 0; i < m_CurveInfoList.Count; ++i)
        {
            if (m_CurveInfoList[i].m_CurveName == curveName)
                return m_CurveInfoList[i];
        }
    ...
    public void SortCurveInfo()
    {
        ...
        for (int i = 0; i < m_CurveInfoList.Count; ++i)
        {
            m_CurveInfoList[i].m_nSortGroup = NcComparerCurve.GetSortGroup(m_CurveInfoList[i]);
        }
    }
