...
public class NcSpriteAnimation : NcEffectAniBehaviour
{
    ...
    public int m_nSelectFrame; // m_nSpriteFactoryIndex
    ...
    [HideInInspector] public bool m_bBuildSpriteObj;
    [HideInInspector] public bool m_bNeedRebuildAlphaChannel;

    [HideInInspector] public AnimationCurve m_curveAlphaWeight;
    // 	[HideInInspector]
    ...
    public override int GetAnimationState()
    {
        if (!enabled || !IsActive(gameObject))
            return -1;
        if (m_fStartTime == 0 || !IsEndAnimation())
            return 1;
        return 0;
    }
    ...
    {
        return m_TextureType == TEXTURE_TYPE.TileTexture ? m_nTilingX * m_nTilingY : m_NcSpriteFrameInfos.Length;
    }

    public int GetValidFrameCount()
    {
        return m_TextureType == TEXTURE_TYPE.TileTexture
            ? m_nTilingX * m_nTilingY - m_nStartFrame
            : m_NcSpriteFrameInfos.Length - m_nStartFrame;
    }
    ...
    void Update()
    {
        ...
        if (m_PlayMode != PLAYMODE.RANDOM)
        {
            ...
            {
                if ((m_PlayMode == PLAYMODE.PINGPONG ? m_nFrameCount * 2 - 1 : m_nFrameCount) <= nSeqIndex
                ) // first loop
                {
                    if (!m_bLoop)
                    {
                    ...
                    else
                    {
                        if (m_PlayMode == PLAYMODE.PINGPONG)
                        {
                            if (m_NcSpriteFactoryCom != null && nSeqIndex % (m_nFrameCount * 2 - 2) == 1)
                                if (m_NcSpriteFactoryCom.OnAnimationLastFrame(this,
                                    nSeqIndex / (m_nFrameCount * 2 - 1)))
                                    return;
                        }
                        else
                        {
    ...
    int CalcPartLoopInfo(int nSeqIndex, ref int nLoopCount)
    {
        ...
        for (int n = nLastLoopCount; n < Mathf.Min(nLoopCount, m_nLoopFrameCount - 1); n++, cnt++)
        {
            ...
            if (transform.parent != null)
                transform.parent.SendMessage("OnSpriteAnimationLoopStart", nLoopCount,
        ...
        if (0 < m_nLoopingCount && m_nLoopingCount <= nLoopCount)
        {
            ...
            if (transform.parent != null)
                transform.parent.SendMessage("OnSpriteAnimationLoopEnd", nLoopCount,
                    SendMessageOptions.DontRequireReceiver);
    ...
    void SetIndex(int nSeqIndex)
    {
        if (m_Renderer != null)
        {
            ...
            switch (m_PlayMode)
            {
                ...
                {
                    nSetIndex = m_bLoop
                        ? CalcPartLoopInfo(nSeqIndex, ref nLoopCount) % m_nFrameCount
                        : nSeqIndex % m_nFrameCount;
                    break;
                }
                case PLAYMODE.INVERSE:
                    nSetIndex = m_nFrameCount - (nSetIndex % m_nFrameCount) - 1;
                    break;
            ...
            if (m_TextureType == TEXTURE_TYPE.TileTexture)
            {
                ...
                if (!UpdateMeshUVsByTileTexture(new Rect(offset.x, offset.y, m_size.x, m_size.y)))
                {
                    m_Renderer.material.mainTextureOffset =
                        new Vector2(offset.x - ((int) offset.x), offset.y - ((int) offset.y));
            ...
            if (m_OnChangeFrameComponent != null)
                m_OnChangeFrameComponent.SendMessage(m_OnChangeFrameFunction, nSetIndex,
                    SendMessageOptions.DontRequireReceiver);
    ...
    public bool UpdateFactoryMaterial()
    {
        if (m_NcSpriteFactoryPrefab == null)
            return false;
        if (m_NcSpriteFactoryPrefab.GetComponent<Renderer>() == null ||
            m_NcSpriteFactoryPrefab.GetComponent<Renderer>().sharedMaterial == null ||
            m_NcSpriteFactoryPrefab.GetComponent<Renderer>().sharedMaterial.mainTexture == null)
            return false;
        ...
        if (m_NcSpriteFactoryCom.m_SpriteType != NcSpriteFactory.SPRITE_TYPE.NcSpriteAnimation &&
            m_NcSpriteFactoryCom.m_SpriteType != NcSpriteFactory.SPRITE_TYPE.Auto)
            return false;
