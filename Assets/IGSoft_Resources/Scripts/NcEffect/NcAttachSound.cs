...
public class NcAttachSound : NcEffectBehaviour
{
    ...
    {
        if ((enabled && IsActive(gameObject)) || (m_AudioSource != null &&
                                                  (m_AudioSource.isPlaying ||
                                                   (GetEngineTime() < m_fStartTime + m_fDelayTime))))
            return 1;
    ...
    {
        if (m_PlayType == PLAY_TYPE.MultiPlay || !m_bSharedAudioSource)
        {
        ...
        {
            NsSharedManager.Inst.PlaySharedAudioSource((m_PlayType == PLAY_TYPE.UniquePlay), m_AudioClip, m_nPriority,
                m_bLoop, m_fVolume, m_fPitch);
        }
