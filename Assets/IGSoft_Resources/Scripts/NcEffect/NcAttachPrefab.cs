...
public class NcAttachPrefab : NcEffectBehaviour
{
    ...
    [HideInInspector] public bool m_bDetachParent;
    ...
    void CreateAttachPrefab()
    {
        ...
        if ((0 == m_fRepeatTime || m_AttachType == AttachType.Destroy) && 0 < m_nRepeatCount &&
            m_nCreateCount < m_nRepeatCount)
            CreateAttachPrefab();
    }

    void CreateAttachGameObject()
    {
        GameObject createObj = CreateGameObject(GetTargetGameObject(),
            (GetTargetGameObject() == gameObject ? null : transform), m_AttachPrefab);
        if (m_bReplayState)
        ...
        createObj.transform.position = m_AddStartPos +
                                       new Vector3(Random.Range(-m_RandomRange.x, m_RandomRange.x) + newPos.x,
    ...
    {
        // ������ǥ �������̸�..
        return m_bWorldSpace || m_AttachType == AttachType.Destroy ? GetRootInstanceEffect() : gameObject;
    }
