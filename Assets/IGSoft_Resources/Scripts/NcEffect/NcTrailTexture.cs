        ...
        if (pointsToUse.Count > 1)
        {
            ...
            for (int n = 0; n < pointsToUse.Count; ++n)
            {
                ...
                if (m_Colors != null && m_Colors.Length > 0)
                {
                    ...
                    if (min >= m_Colors.Length) min = m_Colors.Length - 1;
                    if (min < 0) min = 0;
                    if (max >= m_Colors.Length) max = m_Colors.Length - 1;
                    if (max < 0) max = 0;
                ...
                if (m_SizeRates != null && m_SizeRates.Length > 0)
                {
                    ...
                    if (min >= m_SizeRates.Length) min = m_SizeRates.Length - 1;
                    if (min < 0) min = 0;
                    if (max >= m_SizeRates.Length) max = m_SizeRates.Length - 1;
                    if (max < 0) max = 0;
