...
public class NcBillboard : NcEffectBehaviour
{
    ...
    void Update()
    {
        ...
        // ī�޶� �����͸� �����ϰ� ������ �����͸� �����Ѵ�
        vecUp = m_bFixedObjectUp ? transform.up : Camera.main.transform.rotation * Vector3.up;
        ...
        if (m_RatationMode == ROTATION.RND)
            transform.localRotation *= Quaternion.Euler((m_RatationAxis == AXIS.X ? m_fRndValue : 0),
                (m_RatationAxis == AXIS.Y ? m_fRndValue : 0), (m_RatationAxis == AXIS.Z ? m_fRndValue : 0));
        if (m_RatationMode == ROTATION.ROTATE)
        {
            float fRotValue = GetEngineDeltaTime() * m_fRotationValue;
            transform.Rotate((m_RatationAxis == AXIS.X ? fRotValue : 0), (m_RatationAxis == AXIS.Y ? fRotValue : 0),
                (m_RatationAxis == AXIS.Z ? fRotValue : 0), Space.Self);
