...
public class NcParticleSystem : NcEffectBehaviour
{
...
#if UNITY_EDITOR
        Debug.LogWarning(
            "Waring!!! FXMaker - NcParticleSystem.cs->m_bScaleWithTransform : This option is very slow. (see : Assets/IGSoft_Tools/FXMaker/Readme/Optimization)");
#endif
            ...
            if (IsShuriken())
            {
                if (m_ps != null)
                {
                    ...
                    for (int n = 0; n < m_ps.particleCount; n++)
                    {
                        bool bDestect = false;
                        pos = m_bWorldSpace
                            ? m_BufColliderConParts[n].position
                            : transform.TransformPoint(m_BufColliderConParts[n].position);
                        ...
                        if (m_ParticleDestruct == ParticleDestruct.COLLISION)
                        {
                            ...
                            {
                                if (colls[i].gameObject.GetComponent("FxmInfoIndexing") != null)
                                    continue;
                                bDestect = true;
                                break;
                            }
    ...
    void CreateAttachPrefab(Vector3 position, float size)
    {
        if (m_AttachPrefab == null)
            return;
        GameObject createGameObject = (GameObject) CreateGameObject(m_AttachPrefab,
            m_AttachPrefab.transform.position + position, m_AttachPrefab.transform.rotation);
    ...
    {
        return m_fLegacyMinMeshNormalVelocity *
               (m_bScaleWithTransform ? NcTransformTool.GetTransformScaleMeanValue(transform) : 1);
    }

    public float GetScaleMaxMeshNormalVelocity()
    {
        return m_fLegacyMaxMeshNormalVelocity *
               (m_bScaleWithTransform ? NcTransformTool.GetTransformScaleMeanValue(transform) : 1);
    }
    ...
    public Particle[] LegacyScaleParticle(Particle[] parts, bool bScale, bool bPosUpdate)
    {
        ...
        fScale = bScale
            ? NcTransformTool.GetTransformScaleMeanValue(transform)
            : 1 / NcTransformTool.GetTransformScaleMeanValue(transform);
    ...
    public ParticleSystem.Particle[] ShurikenScaleParticle(ParticleSystem.Particle[] parts, int nCount, bool bScale,
        bool bPosUpdate)
    {
        float fScale;
        if (bScale)
    ...
    {
        PropertyInfo info = srcObj.GetType()
            .GetProperty(fieldName, BindingFlags.Public | BindingFlags.Instance | BindingFlags.NonPublic);
    ...
    public static void SetPropertyValue(SerializedProperty sp, object value)
    {
        switch (sp.propertyType)
        {
            ...
            case SerializedPropertyType.ObjectReference:
                sp.objectReferenceValue = (Object) value;
                break;
