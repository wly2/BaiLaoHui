...
public class NcParticleSpiral : NcEffectBehaviour
{
    ...
    {
        if ((m_ParticlePrefab == null || m_ParticlePrefab.GetComponent<ParticleEmitter>() == null) &&
            (GetComponent<ParticleEmitter>() == null))
    ...
    void Update()
    {
        ...
        if (m_fTurnSpeed != 0)
            transform.Rotate(transform.up * GetEngineDeltaTime() * (m_fTurnSpeed), Space.World);
    }
    ...
    private void KillCurrentEffects()
    {
        ...
        {
            Debug.Log("resetEffect killing: " + emitters[i].name);
            // Make sure autodestruct is on.
            ParticleAnimator animator = emitters[i].transform.GetComponent<ParticleAnimator>();
            if (animator != null)
                animator.autodestruct = true;
            ...
            Particle[] p = emitters[i].particles;
            for (int j = 0; j < p.Length; j++)
                p[j].energy = 0.1f;
            emitters[i].particles = p;
        }
    }
