...
namespace AssemblyCSharp
{
    public class ClientRequest
    {
        ...
        public byte[] ToBytes()
        {
            ...
            {
                BinaryWriter binaryWriter = new BinaryWriter(memoryStream, Encoding.Default); //以二进制写入器往这个流里写内容
                messageContentLength = (short) Encoding.UTF8.GetBytes(messageContent).Length;
                Len = messageContentLength > 0 ? (6 + messageContentLength) : (4 + messageContentLength);
